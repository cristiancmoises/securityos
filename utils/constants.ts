...
// For TRUSTED same-origin embedded apps (kiwiirc) and for navigating the
// in-OS Browser to REMOTE sites: there the framed document has its own (cross-)
// origin, so allow-same-origin lets it use its own storage without being able to
// touch the SecurityOS origin (blocked by the same-origin policy).
...
// For UNTRUSTED, embedder-origin content rendered via srcDoc (e.g. previewing a
// local .html file in the Browser). srcDoc inherits the embedder's origin, so
// allow-same-origin + allow-scripts together would let it script SecurityOS and
// escape the sandbox. Dropping allow-same-origin forces a unique opaque origin.
export const SANDBOXED_IFRAME_CONFIG = {
  referrerPolicy: "no-referrer" as React.HTMLAttributeReferrerPolicy,
  sandbox:
    "allow-downloads allow-forms allow-modals allow-pointer-lock allow-popups allow-presentation allow-scripts",
};

// Tor-Browser "Safest" mode: NO allow-scripts (and no allow-same-origin), so the
// proxied page cannot run any JavaScript at all — the browser-enforced complement
// to the proxy's script stripping + CSP script-src 'none'. Used by the TorBrowser
// app with JS disabled (the default).
export const NOSCRIPT_IFRAME_CONFIG = {
  referrerPolicy: "no-referrer" as React.HTMLAttributeReferrerPolicy,
