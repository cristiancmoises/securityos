      ...
      getInfoByFileExtension("/System/Icons/tinymce.webp", (signal) =>
        fs.readFile(path, async (error, contents = Buffer.from("")) => {
          if (!error && contents.length > 0 && !signal.aborted) {
            ...
            // .whtml is untrusted, user-controlled file content. Rendering it into
            // the live document via innerHTML would fire inline event handlers
            // (e.g. <img onerror>) against the SecurityOS origin — a zero-click
            // stored XSS. Sanitize (fail-closed) before it ever touches the DOM.
            const { sanitizeHtmlString } = await import("utils/sanitize");
            const safeHtml = await sanitizeHtmlString(contents.toString());
