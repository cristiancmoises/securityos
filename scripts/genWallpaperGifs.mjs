...
const codeRain = () => {
  ...
  for (let i = 0; i < cols; i += 1) {
    ...
    const glyphs = [];
    for (let k = 0; k < rows; k += 1) glyphs.push(GLYPHS[Math.floor(r() * GLYPHS.length)]);
    ...
    for (let i = 0; i < cols; i += 1) {
      ...
      for (let rIdx = 0; rIdx < rows; rIdx += 1) {
        ...
        if (d < 1) {
          ...
          g += `<text x="${x}" y="${y}" font-family="'Sarasa Mono J', monospace" font-size="${size}" fill="#19c24f" opacity="${op.toFixed(2)}">${ch}</text>`;
        }
      }
    }
...
const radarSweep = () => {
  ...
  const blips = [];
  for (let i = 0; i < 9; i += 1) blips.push([r() * Math.PI * 2, 60 + r() * (R - 80)]);
  const defs =
  ...
  return (t) => {
    ...
    let g = `<rect width="${W}" height="${H}" fill="url(#bg)"/>`;
    for (let i = 1; i <= 5; i += 1)
      g += `<circle cx="${cx}" cy="${cy}" r="${(i / 5) * R}" fill="none" stroke="#1d9c8a" stroke-width="2" opacity="0.45"/>`;
    for (let i = 0; i < 12; i += 1) {
      const a = (i / 12) * Math.PI * 2;
      g += `<line x1="${cx}" y1="${cy}" x2="${(cx + Math.cos(a) * R).toFixed(1)}" y2="${(cy + Math.sin(a) * R).toFixed(1)}" stroke="#1d9c8a" stroke-width="1" opacity="0.28"/>`;
    }
    // sweep wedge (trailing ~55deg)
    const a0 = ang;
    const a1 = ang - 0.96;
    g += `<path d="M${cx} ${cy} L${(cx + Math.cos(a0) * R).toFixed(1)} ${(cy + Math.sin(a0) * R).toFixed(1)} A${R} ${R} 0 0 0 ${(cx + Math.cos(a1) * R).toFixed(1)} ${(cy + Math.sin(a1) * R).toFixed(1)} Z" fill="url(#sweep)"/>`;
    g += `<line x1="${cx}" y1="${cy}" x2="${(cx + Math.cos(a0) * R).toFixed(1)}" y2="${(cy + Math.sin(a0) * R).toFixed(1)}" stroke="#9bffe6" stroke-width="2.5"/>`;
    ...
    for (const [ba, bd] of blips) {
      ...
      g += `<circle cx="${bx.toFixed(1)}" cy="${by.toFixed(1)}" r="${(4 + near * 6).toFixed(1)}" fill="#5dffd6" opacity="${(0.2 + near * 0.8).toFixed(2)}"/>`;
    }
...
const gridDrive = (opt) => {
  ...
  const defs =
    `<linearGradient id="sky" x1="0" y1="0" x2="0" y2="1">${opt.sky
      .map((c, i) => `<stop offset="${(i / (opt.sky.length - 1)) * 100}%" stop-color="${c}"/>`)
      .join("")}</linearGradient>` +
    `<radialGradient id="glow" cx="50%" cy="0%" r="60%" gradientUnits="userSpaceOnUse" fx="${W / 2}" fy="${horizon}"><stop offset="0%" stop-color="${opt.glow}" stop-opacity="0.5"/><stop offset="100%" stop-color="${opt.glow}" stop-opacity="0"/></radialGradient>` +
    (sun
      ? `<radialGradient id="sun" cx="50%" cy="50%" r="50%" gradientUnits="userSpaceOnUse" fx="${W / 2}" fy="${horizon - 70}"><stop offset="0%" stop-color="#fff2b0"/><stop offset="55%" stop-color="#ffd24d"/><stop offset="100%" stop-color="#ff5d8f"/></radialGradient>`
      : "");
  return (t) => {
    let g = `<rect width="${W}" height="${horizon}" fill="url(#sky)"/>`;
    g += `<rect y="${horizon}" width="${W}" height="${H - horizon}" fill="#05030c"/>`;
    if (sun) {
      g += `<circle cx="${W / 2}" cy="${horizon - 70}" r="150" fill="url(#sun)"/>`;
      for (let i = 0; i < 6; i += 1)
        g += `<rect x="${W / 2 - 155}" y="${horizon - 40 + i * 22}" width="310" height="${6 + i * 2}" fill="${opt.sky[0]}" opacity="0.9"/>`;
    }
    // horizon glow band
    g += `<rect width="${W}" height="${H}" fill="url(#glow)"/>`;
    g += `<rect x="0" y="${horizon - 2}" width="${W}" height="3" fill="${opt.glow}"/>`;
    ...
    for (let i = 0; i < N; i += 1) {
      ...
      const wdt = 0.6 + z * 2.6;
      lines += `<line x1="0" y1="${y.toFixed(1)}" x2="${W}" y2="${y.toFixed(1)}" stroke="${opt.grid}" stroke-width="${wdt.toFixed(2)}" opacity="${op.toFixed(2)}"/>`;
    }
    // converging verticals (static)
    for (let i = -10; i <= 10; i += 1) {
      const bx = W / 2 + (i / 10) * W * 1.5;
      lines += `<line x1="${W / 2}" y1="${horizon}" x2="${bx.toFixed(1)}" y2="${H}" stroke="${opt.grid}" stroke-width="1.6" opacity="0.4"/>`;
    }
    g += `<g>${lines}</g>`;
    // stars in sky
    const r = mulberry32(opt.seed || 5);
    for (let i = 0; i < 70; i += 1)
      g += `<circle cx="${(r() * W).toFixed(1)}" cy="${(r() * horizon).toFixed(1)}" r="${(0.5 + r() * 1.6).toFixed(2)}" fill="#ffffff" opacity="${(0.2 + r() * 0.6).toFixed(2)}"/>`;
...
const SPECS = [
  { theme: "Matrix", name: "code-rain", frames: 44, fps: 20, colors: 36, dither: "none", scale: "960:540", make: codeRain },
  { theme: "Security", name: "radar-sweep", frames: 48, fps: 24, colors: 96, make: radarSweep },
  {
    theme: "Anime",
    name: "neon-drive",
    frames: 48,
    fps: 24,
    colors: 128,
    make: () => gridDrive({ sky: ["#241047", "#7a2a78", "#ff5d73"], grid: "#ff5db0", glow: "#ff9ee0", sun: true, seed: 9 }),
  },
  {
    theme: "Technology",
    name: "grid-corridor",
    ...
    fps: 24,
    colors: 128,
    make: () => gridDrive({ sky: ["#04091a", "#061634", "#020610"], grid: "#28b6ff", glow: "#7cf0ff", sun: false, seed: 3 }),
  },
];
...
const run = () => {
  for (const spec of SPECS) {
    ...
    execFileSync("ffmpeg", [
      ...
      "-i", join(frameDir, "f%03d.png"),
      "-vf", `${spec.scale ? `scale=${spec.scale}:flags=lanczos,` : ""}split[s0][s1];[s0]palettegen=max_colors=${spec.colors}:stats_mode=full[p];[s1][p]paletteuse=dither=${spec.dither || "bayer:bayer_scale=3"}:diff_mode=rectangle`,
      "-loop", "0",
      outGif,
    ]);
    console.log(`  ${spec.theme}/${spec.name}.gif  ${(statSync(outGif).size / 1024 / 1024).toFixed(2)}MB`);
  }
