...
// Best-effort secure wipe of a Web Storage area: overwrite every value with
// CSPRNG data (multiple passes) BEFORE clearing, so a casual dump of the storage
// can't recover prior contents. NOTE: browsers abstract physical storage, so this
// cannot guarantee disk-level erasure — that's an OS/full-disk-encryption concern.
const secureWipeWebStorage = (storage: Storage): void => {
  try {
    const keys = Object.keys(storage);

    for (let pass = 0; pass < 3; pass += 1) {
      keys.forEach((key) => {
        const length = Math.max((storage.getItem(key) || "").length, 32);
        ...
        for (let index = 0; index < bytes.length; index += 1) {
          random += String.fromCharCode(33 + (bytes[index] % 94));
        }
...
export const resetStorage = (rootFs?: RootFileSystem): Promise<void> =>
  new Promise((resolve, reject) => {
    ...
    // Overwrite with randomness before clearing (anti-casual-recovery). The
    // virtual filesystem (IndexedDB) is deleted below.
    secureWipeWebStorage(window.localStorage);
    secureWipeWebStorage(window.sessionStorage);
